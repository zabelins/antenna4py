...
class Syntnet:
    ...
    def calc_out(self, out_set, out_env, out_array1nd, out_proc2nd):
        ...
        for i in range(len_time):
            ...
            self.vec_insignal[i] = np.dot(vec_in[i], vec_inweight)
            self.vec_outsignal[i] = np.dot(vec_in[i], vec_outweight[i])
    ...
    def init_vecmean(self, len_time, len_pattern, len_sig, len_int):
        ...
        self.vec_insignal = np.zeros(shape=[len_time], dtype=complex)
        ...
        self.vec_outsignal = np.zeros(shape=[len_time], dtype=complex)
